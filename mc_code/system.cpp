...
System::System(const double a_size, double e,
  ...
  const int n, int c0, std::vector<Channel*> channels_,
  ...
  num_channels(n),entrance_channel(c0), channels(channels_),
  pot(op), nlpot(nlop), basis_size(n), step_size(step), r_max(max),
  lagrangeBasis(basis_size, a_size), beta(coupling),cmatrix(n,n),
  invcmatrix(n,n), rmatrix(n,n), umatrix(n,n)
{ 
...
//****incomplete*****
arma::cx_double System::couplingPotential(double r1, double r2, Channel * c1, Channel * c2){
  if(c1 == c2){
    return 0;
  }else{
...
void System::waveFunction(std::ofstream& file){
  ...
  file << std::endl << "Printing wave functions" << std::endl;
  int c = 0, cprime = 0;
  std::vector<Channel *>::iterator c1, c2;
  ...
  file //<< "\t\t Total Wave Function" 
    << std::endl;
  file << "-----------------------------------------------------------" << std::endl;
  
  for(double r = 0; r < a; r += step_size){      
    arma::cx_double total_wf(0.0);
    file << r;
    for(c1 = channels.begin(); c1 != channels.end(); c1++){
