    ...
    for(int j = 0; j < N; j++)
      {
       ...
       table[i][j] =interpola2D_cmpx(funcion,r1,r2,
a*N_basis[i],a*N_basis[j],N,N);
...
std::complex < double >** total_matrix_2(std::complex<double>** matrix,double u,double E,int N,double a,int l,double j,int n,int z1, int z2,double B)
  ...
  //  total_matrix= local_B_matrix(m1,m2,N,a,B)+local_potential_matrix(m1,m2,N,a,l,n,z,Vv, rv,av,Wv,rwv,awv,Vd,rvd,avd,Wd,rwd,awd);
   ...
   for ( int c = 0 ; c < N ; c++ )
      ...
      for ( int d = 0 ; d < N ; d++ )
         {
         part= matrix[c][d]+ B_matrix[c][d]+E_matrix[c][d]+S_matrixx[c][d];
         table[c][d] = part;
        // cout<<part;
         }
       }
...
Final_return non_local_wavefunction_matrixx(double u,double E,int N,double a,int l,double j,int n,int z1,int z2, double B, double Nr, double Rmax,Final_return m)
{
  ...
  return non_local_wavefunction_matrix(original,list,list,u,E,N,a,l,j,n,z1,z2, B,Nr,Rmax,m);
...
{ 
 ...
 int start_s=clock(); 
  ...
  MyClass f1;
  MyClass f23;
  std::vector<MyClass> list1;
  std::vector<MyClass> list2;
  std::vector<MyClass> list3;
  std::vector<MyClass> list4;
    const   complex<double> i(0.0,1.0); 
 double m1=1.0;
 double m2=48.0;
...
double exam1[number_of_lines];// array that can hold 100 numbers for 1st column 
double exam2[number_of_lines];// array that can hold 100 numbers for 2nd column 
double exam3[number_of_lines];// array that can hold 100 numbers for 3rd column  
double exam4[number_of_lines];// array that can hold 100 numbers for 3rd column  

ifstream infile;   

  int num = 0; // num must start at 0
  infile.open("pot_p3.txt");// file containing numbers in 3 columns 
...
*/
 ...
 double (*r)[3] = reinterpret_cast<double (*)[3]>(a);  
 double (*matrix_real)[number_of_array]= reinterpret_cast<double (*)[number_of_array]>(exam3);
 double (*matrix_imag)[number_of_array]= reinterpret_cast<double (*)[number_of_array]>(exam4);
  for ( int c = 0 ; c <  number_of_array ; c++ )
      {
      ...
      for ( int d = 0 ; d <  number_of_array ; d++ )
       ...
       //  matrix[c][d].real() = matrix_real[c][d];
        // matrix[c][d].imag() = matrix_imag[c][d];
        // cout<<part;
         }
       }
  for(int x=0;x<3;x++)  // loop 3 times for three lines
