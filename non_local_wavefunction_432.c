...
double V_sp_potential(double Vso,double Rso,double aso,double r,double j,int l)

{

   double Vspin;
   Vspin=(j*(j+1.0)-l*(l+1.0)-0.5*(0.5+1.0))*(Vso/(aso*r))*(pow(hbarc/139.6,2))*(exp((r-Rso)/aso))/(pow((1.0+exp((r-Rso)/aso)),2.0));
...
double W_sp_potential(double Wso,double Rwso,double awso,double r,double j,int l)

{
 
   double Wspin;
   Wspin=(j*(j+1.0)-l*(l+1.0)-0.5*(0.5+1.0))*(Wso/(awso*r))*(pow(hbarc/139.6,2))*(exp((r-Rwso)/awso))/(pow((1.0+exp((r-Rwso)/awso)),2.0));
...
{  
    ...
    return V_potential(Vv,R,n,z1,rv,av)+1.0*i*Wv_potential(Wv,R,n,z1,rwv,awv)+Vd_potential(Vd,R,n,z1,rvd,avd)+1.0*i*Wd_potential(Wd,R,n,z1,rwd,awd)+V_sp_potential(Vso,Rso,aso,R,j,l)+1.0*i*W_sp_potential(Wso,Rwso,awso,R,j,l)+columb_potential(z1,z2,R,n);

}


std::complex < double >** local_potential_matrix(int N,double a,int l,double j, int n,int z1,int z2,double Vv,double rv,double av,double Wv,double rwv,double awv,double Vd,double rvd,double avd,double Wd,double rwd,double awd,double Vso,double Rso,double aso,double Wso,double Rwso,double awso)
...
{      
       ...
       table[i][j] = local_potential(a*N_basis[i],l,j,n,z1,z2,Vv,rv,av,Wv,rwv,awv,Vd,rvd,avd,Wd,rwd,awd,Vso,Rso,aso,Wso,Rwso,awso);
...
std::complex < double > non_local(double r_minus,double r_plus,int l,double j,int n,int z1,int z2, double beta,double Vv1,double rv1,double av1,double Wv1,double rwv1,double awv1,double Vd1,double rvd1,double avd1,double Wd1,double rwd1,double awd1,double Vso1,double Rso1,double aso1,double Wso1,double Rwso1,double awso1)
    {
    ...
    V_potential_average=local_potential(r_average,l,j,n,z1,z2,Vv1,rv1,av1,Wv1,rwv1,awv1,Vd1,rvd1,avd1,Wd1,rwd1,awd1,Vso1,Rso1,aso1,Wso1,Rwso1,awso1);
    if ( abs(zz)<=700)
      {  complex <double> part_b,part_a;
         gsl_sf_result res;
         int rrr=gsl_sf_bessel_il_scaled_e (l, -1.0*zz, & res);
         part_a=1.0*i*res.val*exp(zz);
 ...
 for(int i = 0; i < N; i++) 
    ...
    for(int j = 0; j < N; j++)
      {  
        ...
        if (i==j)
        {
       part1=local_potential(a*N_basis[i],l,j,n,z1,z2,Vv,rv,av,Wv,rwv,awv,Vd,rvd,avd,Wd,rwd,awd,Vso,Rso,aso,Wso,Rwso,awso);
...
{ 
    ...
    for ( N=68; N<70; N=N+2)
     {double T1;
      ...
      mu=40.0/41.0;
      matrix_x=total_potential_matrix(N,15,1,0.5,20,20, 0,50.0,1.25,0.25,0.0,1.25,0.25,0,1.25,0.25,0,1.25,0.25,0,1.25,0.25,0,1.25,0.25,0.85,50.0,1.25,0.25,0.0,1.25,0.25,0,1.25,0.25,0,1.25,0.25,0,1.25,0.25,0,1.25,0.25);
      non_local_wavefunction_matrixx_in(matrix_x,mu,25,N,15,1,0.5,20,20,0,0,0.01,20,b);
