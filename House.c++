namespace Householder_QL
{
  ...
  void one_eigenpairs_inverse_iteration (class matrix<TYPE> &A,
					 TYPE &eigenvalue_guess ,class vector_class<TYPE> &eigenvect_guess,int nb_loop);
				 ...
				 class vector_class<TYPE> * eigenvectors[])
{
  ...
  do
  {
    while ((real (eigenvalues(i_sort)) < pivot_eigenvalue) && (i_sort < N-1))
      i_sort++;

    while ((real (eigenvalues(j_sort)) > pivot_eigenvalue) && (j_sort > 0))
      j_sort--;
				  ...
				  ,class vector_class<TYPE> * eigenvectors[])
{
  ...
  do
  {
    while ( ( abs( real (eigenvalues(i_sort))  ) < pivot_eigenvalue) && (i_sort < N-1))
      i_sort++;

    while ( ( abs(real (eigenvalues(j_sort))  ) > pivot_eigenvalue) && (j_sort > 0))
      j_sort--;
				  ...
				  ,class vector_class<TYPE> * eigenvectors[])
{
  ...
  do
  {
    while ( ( abs( (eigenvalues(i_sort))  ) < pivot_eigenvalue) && (i_sort < N-1))
      i_sort++;

    while ( ( abs( (eigenvalues(j_sort))  ) > pivot_eigenvalue) && (j_sort > 0))
      j_sort--;
...
{ 
  ...
  for (unsigned int loop = 0 ; loop <=nb_loop-1 ; loop++)
    {
      ...
      for (unsigned int ii = 0 ; ii < N ; ii++)
	{
      ...
      // TYPE temp_eigenvalue=(eigenvalue_guess+1.0)*(eigenvect_guess*temp_vect_guess);
      ...
      cout<<" eigenvalue_guess "<<eigenvalue_guess<<endl;
