    ...
    // std::complex <double> mu;
     ...
     for(int j = 0; j < N; j++)
       ...
       if (tt<a)
      {
       ...
       if (tt==a*N_basis[j])
       {
       xuu= pow(-1.0,N+j+1)*sqrt(a*N_basis[j]*(1.0-N_basis[j]));
    
        }
       else 
       {
        xuu= pow(-1.0,N+j+1)*tt/(a*N_basis[j])*sqrt(a*N_basis[j]*(1.0-N_basis[j]))*Legendre(N,2.0*tt/a-1.0)/(tt-a*N_basis[j]);
              
        }  
       add_0=add_0+a_matrix[j]*xuu;
...
struct Config 
{
    int NN;double u;double E;double a_size;int l;double jj; int n;int z1;int z2;int B; double N_r; double R_max;
};

struct Local
{
  double V_v;double r_v;double a_v;double W_v;double r_wv;double a_wv;double V_d; double r_vdd;double a_vd;double W_d;double r_wd;double a_wd;double V_so;double R_so;double a_so;double W_so;double R_wso;double a_wso;
};

struct Non_local 
{
   double beta;double V_v1;double r_v1;double a_v1;double W_v1;double r_wv1;double a_wv1;double V_d1;double r_vdd1;double a_vd1;double W_d1;double r_wd1;double a_wd1;double V_so1;double R_so1;double a_so1;double W_so1;double R_wso1;double a_wso1;
};

void loadConfig(Config& config, Local& local, Non_local& non_local) {
    ifstream fin("tt.txt");
    string line;
    while (getline(fin, line)) {
        ...
        if (line.find("NN") != -1)
            ...
            sin >> config.E;
       else if (line.find("a_size") != -1)
            sin >> config.a_size;
        else if (line.find("l") != -1)
            sin >> config.l;
       else if (line.find("jj") != -1)
            sin >> config.jj;

       else if (line.find("n") != -1)
            sin >> config.n;

       else if (line.find("z1") != -1)
            sin >> config.z1;
       else if (line.find("z2") != -1)
            sin >> config.z2;
            ...
            sin >> non_local.beta;
         else if (line.find("N_r") != -1)
            sin >> config.N_r;

         else if (line.find("R_max") != -1)
            sin >> config.R_max;
      else if (line.find("V_v") != -1)
            sin >> local.V_v;
       else if (line.find("r_v") != -1)
            sin >> local.r_v;
       else if (line.find("a_v") != -1)
            sin >> local.a_v;


      else if (line.find("W_v") != -1)
            sin >> local.W_v;
       else if (line.find("r_wv") != -1)
            sin >> local.r_wv;
       else if (line.find("a_wv") != -1)
            sin >> local.a_wv;

     else if (line.find("V_d") != -1)
            sin >> local.V_d;
       else if (line.find("r_vdd") != -1)
            sin >> local.r_vdd;
     else if (line.find("a_vd") != -1)
           sin >> local.a_vd;




   else if (line.find("W_d") != -1)
            sin >> local.W_d;
       else if (line.find("r_wd") != -1)
            sin >> local.r_wd;
       else if (line.find("a_wd") != -1)
            sin >> local.a_wd;

  else if (line.find("V_so") != -1)
            sin >> local.V_so;
       else if (line.find("R_so") != -1)
            sin >> local.R_so;
       else if (line.find("a_so") != -1)
            sin >> local.a_so;

      else if (line.find("W_so") != -1)
            sin >> local.W_so;
       else if (line.find("R_wso") != -1)
            sin >> local.R_wso;
       else if (line.find("a_wso") != -1)
            sin >> local.a_wso;
      else if (line.find("V_v1") != -1)
            sin >> non_local.V_v1;
       else if (line.find("r_v1") != -1)
            sin >>  non_local.r_v1;
       else if (line.find("a_v1") != -1)
            sin >>  non_local.a_v1;


      else if (line.find("W_v1") != -1)
            sin >>  non_local.W_v1;
       else if (line.find("r_wv1") != -1)
            sin >> non_local.r_wv1;
       else if (line.find("a_wv1") != -1)
            sin >>  non_local.a_wv1;

     else if (line.find("V_d1") != -1)
            sin >>  non_local.V_d1;
       else if (line.find("r_vdd1") != -1)
            sin >>  non_local.r_vdd1;
       else if (line.find("a_vd1") != -1)
            sin >>  non_local.a_vd1;




   else if (line.find("W_d1") != -1)
            sin >>  non_local.W_d1;
       else if (line.find("r_wd1") != -1)
            sin >>  non_local.r_wd1;
       else if (line.find("a_wd1") != -1)
            sin >>  non_local.a_wd1;

  else if (line.find("V_so1") != -1)
            sin >>  non_local.V_so1;
       else if (line.find("R_so1") != -1)
            sin >>  non_local.R_so1;
       else if (line.find("a_so1") != -1)
            sin >> non_local.a_so1;

else if (line.find("W_so1") != -1)
            sin >>  non_local.W_so1;
       else if (line.find("R_wso1") != -1)
            sin >> non_local.R_wso1;
       else if (line.find("a_wso1") != -1)
            sin >>  non_local.a_wso1;
...
{ 
    ...
    outtime.open("example_time_time_NONLOCAL1.txt"); // opens the file
      ...
      mu=40.0/41.0;
  // non_local_wavefunction_matrixx_in(58,mu,25,14,1,0.5,20,20, 0,0,50.0,1.25,0.25,70.0,1.25,0.25,0,1.25,0.25,0,1.25,0.25,0.0,1.25,0.25,0.0,1.25,0.25,0.85,50.0,1.25,0.25,0.0,1.25,0.25,0,1.25,0.25,0,1.25,0.25,0.0,1.25,0.25,0,1.25,0.25,0.01,50,b);
   
     Config config;
     Local local;
     Non_local non_local;
    loadConfig(config,local,non_local);


  /*   cout <<"N"<< config.NN << '\n';
     cout << "u" << config.u << '\n';
      cout<< "E" << config.E << '\n';
    cout << "Rmax" << config.R_max << '\n';
    cout <<" a" << config.a_size << '\n' ;
        cout <<" l" << config.l << '\n' ;
     cout <<" jj" << config.jj << '\n' ;
      cout <<" n" << config.n << '\n' ;
     cout <<"z1" << config.z1 << '\n' ;
     ...
     cout <<" Vv" << config.Vv << '\n' ;
      cout <<"rv" << config.rv << '\n' ;
     cout <<"av" << config.av << '\n' ;*/
 cout<<"   "<<local.r_vdd<<endl;
cout<<" "<<local.a_vd<<endl;
 cout<< " "<< config.NN <<" " << config.u <<" " << config.E <<" " <<endl;
  cout<<" "<< config.a_size <<" " << config.l <<" "<< config.jj <<" " <<config.n <<" "<< config.z1<<" " << endl;
cout<<config.z2 <<" "<< config.B <<" "<<endl;
cout<< local.V_v<<" " << local.r_v<<" "<<endl;
cout<< local.a_v<<" "<<local.W_v<<" "<<endl;
cout<<local.r_wv<<" "<<local.a_wv<<" "<<local.V_d<<" "<<endl;

cout<<" "<<local.W_d<<" "<<local.r_wd<<" "<<local.a_wd<<" "<<local.V_so<<" "<<endl;
cout<<local.R_so<<" "<<local.a_so<<" "<<local.W_so<<" "<<local.R_wso<<" "<<local.a_wso<<" "<<non_local.beta<<" "<<endl;
cout<<non_local.V_v1<<" "<<non_local.r_v1<<" "<<non_local.a_v1<<" "<<non_local.W_v1<<" "<<endl;
cout<<non_local.r_wv1<<" "<<non_local.a_wv1<<" "<<non_local.V_d1<<" "<<endl;
cout<<non_local.r_vdd1<<" "<<non_local.a_vd1<<" "<<non_local.W_d1<<" "<<endl;
cout<<non_local.r_wd1<<" "<<non_local.a_wd1<<" "<<non_local.V_so1<<" "<<non_local.R_so1<<" "<<non_local.a_so1<<" "<<non_local.W_so1<<" "<<non_local.R_wso1<<" "<<non_local.a_wso1<<" "<<endl;
cout<< config.N_r<<" "<<config.R_max<<" "<<endl ;
 //non_local_wavefunction_matrixx_in(config.NN,config.u,config.E,config.a_size,config.l,config.jj, config.n,config.z1,config.z2,config.B,config.V_v,config.r_v,config.a_v,config.W_v,config.r_wv,config.a_wv,config.V_d,config.r_vdd,config.a_vd,config.W_d,config.r_wd,config.a_wd,config.V_so,config.R_so,config.a_so,config.W_so,config.R_wso,config.a_wso,config.beta,config.V_v1,config.r_v1,config.a_v1,config.W_v1,config.r_wv1,config.a_wv1,config.V_d1,config.r_vdd1,config.a_vd1,config.W_d1,config.r_wd1,config.a_wd1,config.V_so1,config.R_so1,config.a_so1,config.W_so1,config.R_wso1,config.a_wso1, config.N_r, config.R_max,b);
    //  int stop_s=clock();
